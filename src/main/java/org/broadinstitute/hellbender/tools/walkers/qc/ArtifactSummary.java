...
public class ArtifactSummary implements Locatable {
    ...
    public ArtifactSummary(final AlignmentContext alignmentContext, final ReferenceContext referenceContext, final int numBasesOnEitherSide, final byte minBaseQuality) {
        ...
        refBases = Arrays.copyOfRange(allBases, basesToDiscardInFront, basesToDiscardInFront + 2 * numBasesOnEitherSide);

        baseCounts = new int[4];
        for (final PileupElement pe : alignmentContext.getBasePileup()) {
            if (pe.isBeforeDeletionStart()) {   // if context is ACGTTTT and we have a deletion GTTTT -> G, then the G is the "before deletion start" base
                deletionStartCount++;
            } else if (pe.isBeforeInsertion()) {
                insertionStartCount++;
            }

            if ( !pe.isDeletion() && pe.getQual() >= minBaseQuality) {
    ...
    private static class ArtifactSummaryTableWriter extends TableWriter<ArtifactSummary> {
        private ArtifactSummaryTableWriter(final File output) throws IOException {
